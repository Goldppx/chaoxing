                ...
                match task.result:
                    ...
                    case ChapterResult.NOT_OPEN:
                        ...
                        if task.tries >= self.max_tries:
                            logger.warning(
                                f"Chapter is not open: {task.point['title']}"
                            )
                            continue
                    ...
                    case ChapterResult.ERROR:
                        task.tries += 1
                        logger.warning(
                            f"Retrying task {task.point['title']} ({task.tries}/{self.max_tries} attempts)"
                        )
                        if task.tries >= self.max_tries:
                            logger.error(
                                f"Max retries reached for task: {task.point['title']}"
                            )
                    ...
                    case _:
                        logger.error(
                            f"Invalid task state {task.result} for task {task.point['title']}"
                        )
                        self.failed_tasks.append(task)
            finally:
